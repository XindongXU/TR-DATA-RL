...
def get_points(m, c):
    ...
    xlin = np.linspace(0, HEIGHT - 1, 30000)
    ylin = m * xlin + c

    in_image = np.logical_and(ylin < WIDTH - 1, ylin >= 0)

    if np.sum(in_image) < 3:
        ylin = np.linspace(0, WIDTH - 1, 30000)
        xlin = - ylin / m - c / m
        in_image = np.logical_and(xlin < HEIGHT - 1, xlin >= 0)
...
def frame_computation(frame):

    mask = find_stick.find_stick_BGR(frame)
...
def identify_stick_video(name, startFrameNumber, finishFrameNumber):
    ...
    rows = np.zeros([finishFrameNumber - startFrameNumber + 1, 5])
    for i in tqdm(range(startFrameNumber, finishFrameNumber)):
        ...
        row = frame_computation(frame)
        rows[i - startFrameNumber] = row

    # fields = "base_x, base_y, slope, c coeff, theta"
    # np.savetxt("test.csv", rows, delimiter=",", header=fields, fmt='%.7f', comments='')


if __name__ == "__main__":

    identify_stick_video("tentacle.mp4", 5000, 5050)
